...
app.use(a.static(path.join(__dirname, "public"))); // moved files into 'public' folder for safety

// it was used to make a structure of the pgm ku using the binary code to normal code
app.use(a.urlencoded({ extended: true }));
app.use(a.json()); // added to handle JSON data too
...
app.get('/login', function (req, res) {
    res.sendFile(path.join(__dirname, "public", "login.html")) // this line is used to reterive the direct login file and also act as a link that show in the chrome
});
